...
export const createCloudbeesInsielTomcatAction = (options: { config: Config }) => {
  ...
  return createTemplateAction<{
    ...
    id: 'krateo:cloudbees-insiel-tomcat',
    schema: {
      input: {
    ...
    async handler(ctx) {
      ...
      const cfgXml = path.join(cbDir, 'cfg.xml');
      ...
      // Configuration files
      const cfgModified = nunjucks.render(cfgXml, {
        owner,
        gitHubUrl: ctx.input.gitHubUrl,
        repo: `${repo}-cfg`,
        guid: uuidv4(),
      });
      fs.writeFileSync(cfgXml, cfgModified, { encoding: 'utf-8' });
      ctx.logger.info(`Created cfg.xml`);
      ...
      await axiosInstance({
        method: 'post',
        url: `${cb.baseUrl}/createItem?name=${ctx.input.component_id}-cfg`,
        data: Buffer.from(fs.readFileSync(cfgXml)),
        headers: {
          Accept: '*/*',
          'Content-Type': `text/xml`,
          Authorization: `Basic ${Buffer.from(token).toString('base64')}`,
      ...
      ctx.logger.info(`Pipeline for configuration files created`);
